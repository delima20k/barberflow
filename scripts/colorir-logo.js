...
const INPUT  = path.join(__dirname, '../shared/img/LogoNomeBarberFlow.png');
const OUTPUT = INPUT;

// Luminância para detectar pixels escuros (texto)
function lum(r, g, b) {
...
(async () => {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (a < 10) continue;
      if (lum(r, g, b) > 100) continue; // fundo claro — ignora
