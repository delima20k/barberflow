...
| `GpsBarbeariaMarker` | [shared/js/GpsBarbeariaMarker.js](shared/js/GpsBarbeariaMarker.js) | interfaces | Cria L.Marker Leaflet premium para barbearias (casinha 3D + logo + popup rico + botão "Ver Barbearia"). Método público: `static criar(barbearia, { posUsuario })` → L.Marker\|null. Despacha CustomEvent('mapa:ver-barbearia'). Consumido por MapWidget. |
...
| ~~`GpsPanelMap`~~ | ~~apps/profissional/assets/js/GpsPanelMap.js~~ | ~~interfaces~~ | **REMOVIDO** — mapa jamais solicitado na página de cadastro de endereço. Script não carregado. Substituído por `GpsBarbeariaMarker` nas homes. |
