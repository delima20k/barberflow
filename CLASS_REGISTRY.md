...
| `FinalizarCorteModal` | [shared/js/FinalizarCorteModal.js](shared/js/FinalizarCorteModal.js) | interfaces | Modal de confirmação de finalização de corte com seleção de método de pagamento. `static abrir({clienteNome,proximoNome})` → `Promise<{confirmado:boolean, paymentMethod:'pix'\|'dinheiro'\|'cartao'\|null}>`. Finalizar desabilitado até método selecionado. |
| `FinanceiroRepository` | [shared/js/FinanceiroRepository.js](shared/js/FinanceiroRepository.js) | infra | CRUD da tabela `transactions`. `criarTransacao({barbershopId,professionalId,clientId,queueEntryId,amount,paymentMethod})`, `getResumoPorPeriodo(shopId,{de,ate})`, `getTotalPeriodo(shopId,{de,ate})`, `getTransacoesBarbeiro(shopId,profId,{de,ate})`. Valida UUIDs via InputValidator. |
| `FinanceiroService` | [shared/js/FinanceiroService.js](shared/js/FinanceiroService.js) | application | Registra corte financeiro (busca preços de queue_entry_services + insere transação) e consultas de resumo/extrato por período. `registrarCorte({entradaId,barbershopId,professionalId,clientId,paymentMethod})`, `getResumo(shopId,periodo)`, `getTransacoesBarbeiro(shopId,profId,periodo)`. Despacha `barberflow:transacao-criada`. |
