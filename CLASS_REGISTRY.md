...
| `GuestFilaModal` | [shared/js/GuestFilaModal.js](shared/js/GuestFilaModal.js) | interfaces | Formulário compacto (nome + WhatsApp opcional, com honeypot) pra visitante sem login entrar na fila. `static abrir()` → `Promise<{nome, telefone}\|null>`. Sem regra de negócio, sem rede — mesmo padrão de overlay/Promise do `CorteModal`. |
| `GuestFilaService` | [shared/js/GuestFilaService.js](shared/js/GuestFilaService.js) | application | Equivalente de `ClienteController.entrarNaFila()` para visitante sem conta. `entrar({barbershopId, professionalId?, guestName, guestPhone?, serviceIds?})` chama `BffApiService.fila.entrarComoConvidado` e persiste a referência via `GuestFilaSessionStore`. Sempre resulta em status `waiting` — endpoint da BFF não cria entrada direto em produção. |
| `GuestFilaSessionStore` | [shared/js/GuestFilaSessionStore.js](shared/js/GuestFilaSessionStore.js) | infra | Único ponto de acesso ao localStorage pra lembrar, por aparelho, qual entrada de fila um visitante sem conta criou (`salvar`, `obter`, `limpar`, chave `bf:fila-convidado:<barbershopId>`). Sem vínculo com conta — perdido ao limpar cache/trocar aparelho (aceito por design). |
...
| `CadeiraService` | [shared/js/CadeiraService.js](shared/js/CadeiraService.js) | application | Lógica de negócio das cadeiras da fila: `getClientesConhecidos(barbershopId)`, `getFilaAtiva(barbershopId)`, `getClientesFavoritos(barbershopId,professionalId)` (delega a `FavoritosClientesService`), `sentar({barbershopId,professionalId,clientId,serviceIds,tipo,notificarBarbeiro?})`, `finalizar(entradaId,barbershopId)`. O controle opcional de notificação diferencia a entrada feita pelo cliente da ação manual do profissional. Despacha `barberflow:cadeira-atualizada`. |
...
| `FilaConvidadoController` | [barberflow-bff-api/controllers/FilaConvidadoController.js](barberflow-bff-api/controllers/FilaConvidadoController.js) | interfaces | `extends BaseController`. Handler de `POST /api/v1/fila/entrar` — rota pública (sem `AuthMiddleware`) para visitante sem conta entrar na fila via link da barbearia. Sempre força `clienteId: null` ao montar o comando, ignorando qualquer `client_id` vindo do corpo. Protegida por `AbuseMiddleware` (5 tentativas/10min por IP). Delega a `EntrarNaFilaUseCase`. |
...
| `EntrarNaFilaUseCase` | [barberflow-bff-api/application/fila/EntrarNaFilaUseCase.js](barberflow-bff-api/application/fila/EntrarNaFilaUseCase.js) | application | Insere cliente (com conta ou convidado — `clienteId` xor `guestName`) na fila. Com `barbeariaRepository` injetado (opcional), valida barbearia aberta e vínculo do profissional; valida serviços escolhidos via `FilaRepository.servicosValidos`; calcula posição via `countAtivos`, persiste, vincula serviços (`linkServicos`) e publica evento. |
| `FilaRepository` | [barberflow-bff-api/infrastructure/db/FilaRepository.js](barberflow-bff-api/infrastructure/db/FilaRepository.js) | infra | `implements IFilaRepository`. Tabela `queue_entries` (colunas reais: `position`, `client_confirmed`, `guest_name`, `guest_phone`; sem `created_at`/`updated_at` — usa `check_in_at`). Métodos: `findById`, `findByBarbershop`, `save`, `delete`, `countAtivos`, `servicosValidos(barbershopId, serviceIds)`, `linkServicos(queueEntryId, barbershopId, serviceIds)` (grava em `queue_entry_services`). |
...
| `MetaPixelTracker` | [apps/landing-page/js/meta-pixel.js](apps/landing-page/js/meta-pixel.js) | infra | Carrega o Meta Pixel somente na landing e envia PageView, VoucherStart, CompleteRegistration e GoToSignup sem parâmetros de dados pessoais ou voucher, com inicialização e conversão idempotentes. |
