...
| `CadeiraConfirmacaoService` | [shared/js/CadeiraConfirmacaoService.js](shared/js/CadeiraConfirmacaoService.js) | application | Orquestra confirmação de presença do cliente na cadeira de produção. `static iniciarFluxo(entradaId, clienteNome)` — toca som, abre modal, chama RPC `confirmar_presenca_cliente`. Grace period de 5 min após 1º "Não". `static restaurar(userId)` → `Promise<boolean>` — reabre modal pendente ao reabrir app (localStorage `bf_confirmacao_pendente`); retorna `true` se localStorage tinha dado, `false` se vazio (sinal para tentar P2P pull). `static parar(entradaId?)`, `static temTimer(entradaId)` (teste). |
| `ConfirmP2PService` | [shared/js/ConfirmP2PService.js](shared/js/ConfirmP2PService.js) | application | Cache P2P de confirmações de corte pendentes. Barbeiro: `armazenarParaCliente(clientId, entradaId, shopId)` grava dado em Map; `iniciarBarber(shopId)` escuta canal Realtime e responde a pull; `pararBarber()` limpa tudo (logout). Cliente: `tentarPull(shopId, clientId, timeout?)` → `Promise<{entradaId}\|null>` — solicita via Broadcast, aguarda resposta, sinaliza done automaticamente. Helper de teste: `_getCacheEntry(clientId)`. Canal: `confirm-pull:{shopId}`. |
