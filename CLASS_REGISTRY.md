...
| `MensalistaModal` | [shared/js/MensalistaModal.js](shared/js/MensalistaModal.js) | interfaces | Modal de gestão de mensalistas da barbearia. `static abrir({barbershopId})` → `Promise<void>`. Lista ativos (nome + vencimento + remover) e busca de clientes disponíveis para adicionar. Prefixo CSS `mslm-`. |
...
| `MensalistaService` | [barberflow-bff-api/services/MensalistaService.js](barberflow-bff-api/services/MensalistaService.js) | application | `extends BaseService`. Regras de negócio de mensalistas. Verifica ownership de barbearia via `#verificarOwnership`. Métodos: `adicionar(userId,barbershopId,clientId)`, `listar(userId,barbershopId)`, `verificar(barbershopId,clientId)` (sem ownership), `remover(userId,id)`, `buscarClientesDisponiveis(userId,barbershopId,q)`. |
