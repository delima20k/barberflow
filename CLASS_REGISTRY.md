...
| `BarbeiroEsperaFluxo` | [shared/js/BarbeiroEsperaFluxo.js](shared/js/BarbeiroEsperaFluxo.js) | application | Fluxo de espera do barbeiro quando cliente ainda não se sentou. Orquestra 2 modais via FluxoDeFila e chama CadeiraService.finalizar se cancelar. `static iniciar({clienteNome, entradaId, barbershopId})` → `Promise<{status:'aguardando'\|'finalizado', proximoNome?}>`. |
