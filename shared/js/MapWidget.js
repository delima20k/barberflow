...
class MapWidget {
  ...
  /**
   * Adiciona marcadores premium para cada barbearia da lista.
   * Renderiza em lotes de 20 via requestAnimationFrame para não bloquear a UI.
   */
  static #renderMarcadores(lista) {
    ...
    const renderBatch = () => {
      const lote = items.splice(0, BATCH);
      lote.forEach(b => {
        const marker = GpsBarbeariaMarker.criar(b, {
          posUsuario: MapWidget.#posUsuario,
        });
        if (marker) marker.addTo(MapWidget.#layerBarbearias);
      });
  ...
  // ═══════════════════════════════════════════════════════════
  // PRIVADO — Helpers de coordenadas
  // ═══════════════════════════════════════════════════════════
